...
$(function () {
    $.get("load_room.do", {}, function (data) {
        for (var i in data) {
            ...
            var td7 = $('<td><button class="btn btn-warning btn-sm" r_id="' + data[i].r_id + '">修改</button></td>');
...
$(function () {
    ...
    $("#room_body").on("click", "button", function () {
        ...
        $.get("load_room_type.do", {}, function (data) {
            ...
            f.r_num.value = _this.parent().parent().eq(2).html();
            f.r_size.value = _this.parent().parent().eq(3).html();
            f.r_typeid.value = _this.parent().parent().eq(1).attr("r_typeid");
            f.r_isbath.value = _this.parent().parent().eq(4).attr("r_isbath");
            f.r_money.value = _this.parent().parent().eq(5).html();
        });
    });
